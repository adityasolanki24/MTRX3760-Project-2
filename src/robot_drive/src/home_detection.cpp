...
RightHandWallFollowerAruco::PIDController::PIDController(double kp, double ki, double kd, double max_output)
: kp_(kp),
  ki_(ki),
  kd_(kd),
...
double RightHandWallFollowerAruco::PIDController::calculate(double error, double dt)
{
    // Handle first call
    if (first_call_) {
        prev_error_ = error;
...
void RightHandWallFollowerAruco::PIDController::reset()
{
    integral_ = 0.0;
    prev_error_ = 0.0;
    first_call_ = true;
...
RightHandWallFollowerAruco::RightHandWallFollowerAruco()
: Node("right_hand_wall_follower_aruco"),
  linear_speed_(0.1),
  angular_speed_(0.4),
  ...
  aruco_id_1_detected_(false),
  last_scan_time_(this->now())
{
    // Parameter: stop distance to ArUco ID 1 (meters)
    aruco_stop_distance_m_ = this->declare_parameter<double>("aruco_stop_distance", 0.30);
    ...
    scan_sub_ = this->create_subscription<sensor_msgs::msg::LaserScan>(
        "scan", qos, std::bind(&RightHandWallFollowerAruco::scan_callback, this, std::placeholders::_1));
    
    // Subscribe to ArUco marker ID 1 pose (from aruco_pose_node)
    aruco_id1_sub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
        "aruco/pose/id_1", 10, std::bind(&RightHandWallFollowerAruco::aruco_id1_callback, this, std::placeholders::_1));
    
    RCLCPP_INFO(this->get_logger(), "============================================================");
    RCLCPP_INFO(this->get_logger(), "Right-Hand Wall Follower - PID CONTROL VERSION WITH ARUCO");
    RCLCPP_INFO(this->get_logger(), "============================================================");
    ...
    RCLCPP_INFO(this->get_logger(), "ArUco Detection: Stop at %.2fm when ID 1 is detected", aruco_stop_distance_m_);
    RCLCPP_INFO(this->get_logger(), "============================================================");
...
float RightHandWallFollowerAruco::find_min(const std::vector<float>& ranges, size_t start, size_t end)
{
...
RightHandWallFollowerAruco::RangeData RightHandWallFollowerAruco::get_scan_ranges(
...
void RightHandWallFollowerAruco::aruco_id1_callback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
{
    ...
    const double distance_m = std::sqrt(x * x + y * y + z * z);

    RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
        "ArUco ID 1 distance: %.3f m (stop at %.2f m)", distance_m, aruco_stop_distance_m_);

    // Stop only when within the configured stop distance
    if (!aruco_id_1_detected_ && distance_m <= aruco_stop_distance_m_) {
        aruco_id_1_detected_ = true;
        RCLCPP_WARN(this->get_logger(), "============================================================");
        RCLCPP_WARN(this->get_logger(), "ArUco Marker ID 1 within %.2fm - STOPPING", aruco_stop_distance_m_);
        RCLCPP_WARN(this->get_logger(), "Position: [%.3f, %.3f, %.3f] (dist=%.3f m)", x, y, z, distance_m);
        RCLCPP_WARN(this->get_logger(), "============================================================");
...
void RightHandWallFollowerAruco::scan_callback(const sensor_msgs::msg::LaserScan::SharedPtr msg)
{
    ...
    // PRIORITY 0: Stop if ArUco marker ID 1 has been detected
    if (aruco_id_1_detected_) {
        state_ = "STOPPED_ARUCO_DETECTED";
        cmd.twist.linear.x = 0.0;
        cmd.twist.angular.z = 0.0;
        wall_pid_->reset();
        RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
            "STOPPED: ArUco marker ID 1 detected. Wall following disabled.");
...
int main(int argc, char** argv)
{
    ...
    std::cout << "\n============================================================\n";
    std::cout << "Wall Follower with ArUco Detection Active\n";
    std::cout << "============================================================\n";
    ...
    std::cout << "ArUco Detection: Will stop when marker ID 1 is detected\n";
    std::cout << "============================================================\n\n";
    
    auto node = std::make_shared<RightHandWallFollowerAruco>();
